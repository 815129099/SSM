...
function e(){
var b=document.getElementById("doughnutChartCanvas");
...
data:{kd:a},dataType:"json"}).done(function(a){c=a}),$(b).autocomplete({source:c})}
...
async:!1,
url:ctx+"/jobs/listLs",
data:{pl:a},dataType:"json"}).done(function(a){a.success&&(c=a.content.result)}),
...
var b=$("#"+a).offset();
switch(a){
case"expectJob":$("#expectJob .expectAdd").trigger("click");break;
case"workExperience":$("#workExperience .experienceAdd").trigger("click");break;
case"projectExperience":$("#projectExperience .projectAdd").trigger("click");break;
case"educationalBackground":$("#educationalBackground .educationalAdd").trigger("click");break;
...
$("body,html").animate({scrollTop:b.top},400,
function(){
...
var a=$("#"+c);
this.AllowExt=".jpg,.gif,.jpeg,.png,.pjpeg",this.FileExt=a.val().substr(a.val().lastIndexOf(".")).toLowerCase(),0!=this.AllowExt&&-1==this.AllowExt.indexOf(this.FileExt)?errorTips("只支持jpg、gif、png、jpeg格式，请重新上传","上传头像"):($("#"+c+"_error").text("").hide(),$.ajaxFileUpload({url:b,secureuri:!1,fileElementId:c,data:{headPic:a.val()},dataType:"json",success:function(a){if(a.success){var b=a.content;$("#portraitShow img").attr("src",ctx+"/"+b),$("#basicInfo .basicShow img").attr("src",ctx+"/"+b),$("#portraitNo").hide(),$("#portraitShow").show()}else errorTips("上传失败，请重新上传","上传头像")},error:function(){errorTips("上传失败，请重新上传","上传头像")}}))}
...
!function(a,b){
function c(a,b,c){
var d=k[b.type]||{};return null==a?c||!b.def?null:b.def:(a=d.floor?~~a:parseFloat(a),isNaN(a)?b.def:d.mod?(a+d.mod)%d.mod:0>a?0:d.max<a?d.max:a)}

function d(b){
var c=i(),d=c._rgba=[];return b=b.toLowerCase(),o(h,function(a,e){var f,g=e.re.exec(b),h=g&&e.parse(g),i=e.space||"rgba";return h?(f=c[i](h),c[j[i].cache]=f[j[i].cache],d=c._rgba=f._rgba,!1):void 0}),d.length?("0,0,0,0"===d.join()&&a.extend(d,n.transparent),c):n[b]}
...
projectMonthEnd:{required:function(){
return"至今"==$(".projectYearEnd").val()?!1:!0},checkProjectYear:!0},projectDescription:{required:!1}},

messages:{
projectName:{required:"请输入项目名称",maxlenStr:"请输入有效的项目名称"},
thePost:{required:"请输入担任职务",maxlenStr:"请输入有效的担任职务"},projectYearStart:{required:"请选择开始时间",checkProjectYear:"开始时间需早于结束时间"},projectMonthStart:{required:"请选择开始时间",checkProjectYear:"开始时间需早于结束时间"},projectYearEnd:{required:"请选择结束时间",checkProjectYear:"结束时间需晚于开始时间"},projectMonthEnd:{required:"请选择结束时间",checkProjectYear:"结束时间需晚于开始时间"}},
...
type:"POST",
data:{projectName:b,positionName:c,startYear:e,startMonth:f,endYear:g,endMonth:h,projectRemark:i,projectid:j,id:l,resubmitToken:m},dataType:"json"}).done(function(b){
var c,e;
$("#resubmitToken").val(b.resubmitToken),
b.success?(c=b.content.projectExperience,e="",
...
messages:{
workLink:{checkUrlNot:"请输入有效的作品链接",maxlength:"请输入有效的作品链接"},workDescription:{maxlength:"请输入100字以内的有效作品说明"}},
...
var c,d,e;
$("#resubmitToken").val(b.resubmitToken),
b.success?(c="",d="",e=b.content.workShow,3==b.code?""!=e?(c+='<li data-id="'+e.id+'"><div class="workList c7">'+'<i class="sm_del dn"></i>'+'<i class="sm_edit dn"></i>',e.url&&(d="http://"==e.url.substring(0,7)||"https://"==e.url.substring(0,8)?e.url:"http://"+e.url,c+='<div class="f16">网址：<a href="'+d+'" target="_blank">'+e.url+"</a></div>"),
c+="<p>"+e.workName+"</p>"+"</div></li>",
placeholderFn(),""!=$.trim(h.obj.children(".workShow").children(".slist").html())&&$(a).remove(),
...
h.obj.children(".c_add").removeClass("dn")):(h.obj.children(".workEdit").addClass("dn"),h.obj.children(".workAdd").removeClass("dn")):4==b.code?(c+='<div class="workList c7"><i class="sm_del dn"></i><i class="sm_edit dn"></i>',e.url&&(d="http://"==e.url.substring(0,7)||"https://"==e.url.substring(0,8)?e.url:"http://"+e.url,c+='<div class="f16">网址：<a href="'+d+'" target="_blank">'+e.url+"</a></div>"),
c+="<p>"+e.workName+"</p>"+"</div>",
placeholderFn(),$(a).parent("li").html(c)):2==b.code&&($(a).parent("li").remove(),
...
var a=$(this).attr("rel");"complete"!=a&&click_scroll(a)}),

jQuery.validator.addMethod("checkWorkYear",function(a,b){
var c=$(b).parents(".experienceForm"),d=c.find(".companyYearStart").val(),e=c.find(".companyMonthStart").val(),f=c.find(".companyYearEnd").val(),g=c.find(".companyMonthEnd").val();return d>f?!1:d==f&&e>g?!1:!0},"开始时间需早于结束时间"),

jQuery.validator.addMethod("checkProjectYear",function(a,b){
var c=$(b).parents(".projectForm"),d=c.find(".projectYearStart").val(),e=c.find(".projectMonthStart").val(),f=c.find(".projectYearEnd").val(),g=c.find(".projectMonthEnd").val();return d>f?!1:d==f&&e>g?!1:!0},"开始时间需早于结束时间"),

jQuery.validator.addMethod("checkSchoolYear",function(a,b){
var c=$(b).parents(".educationalForm"),d=c.find(".schoolYearStart").val(),e=c.find(".schoolYearEnd").val();return d>e?!1:!0},"开始时间需小于结束时间"),
...
c=$("#resumeId").val();
$(a).find(":submit").attr("disabled",!0),
$.ajax({url:ctx+"/resume/renameResume.json",
type:"POST",data:{resumeName:b,id:c}}).done(function(b){
if(b.success){
var c="";
...
var sName = $("#showName").html().split("：");
$(".basicEdit #profileForm #name").val(sName[1]);

var sSex = $("#showSex").html().split("：");
if(sSex[1]=="男"){
$("#boy").attr("checked",true);
$(".girl").removeClass("current");
$(".boy").addClass("current");
}else{
$("#girl").attr("checked",true);
$(".boy").removeClass("current");
$(".girl").addClass("current");
}
var sAge = $("#showAge").html().split("：");
$(".basicEdit #profileForm #age").val(sAge[1]);

var sHe = $("#showHe").html().split("：");
$(".basicEdit #profileForm #height").val(sHe[1]);

var sWe = $("#showWe").html().split("：");
$(".basicEdit #profileForm #weight").val(sWe[1]);

var sPhone = $("#showPhone").html().split("：");
$(".basicEdit #profileForm #tel").val(sPhone[1]);
j($("#basicInfo"))}),
...
messages:{
name:{required:"请输入你的真实姓名",specialchar:"请输入你的真实姓名",checkNum:"请输入你的真实姓名",rangelength:"请输入你的真实姓名"},
...
d=$('input[name="height"]',a).val(),
e=$('input[name="weight"]',a).val(),
...
type:"POST",
url:"comBasicInfo",
data:{email:email,name:b,age:age,sex:c,height:d,weight:e,phone:h,},
dataType:"json",
success:function(response){
if(response.tip==true){
$("#resubmitToken").val(response.resubmitToken),
...
$("#basicInfo .basicEdit").addClass("dn");
var htm = $(".basicShow #showName").html();
if(htm==null || htm.length<=0){
$("#basicInfo .c_edit").addClass("dn");
...
a.stopPropagation();var b=$(this).text();
...
Edit:function(){
""==this.expectCityVal?($("#expectCity").val(""),$("#select_expectCity").val("期望城市，如：北京").css("color","#999")):($("#expectCity").val(this.expectCityVal),
$("#select_expectCity").val(this.expectCityVal).css("color","#333")),
""!=this.typeVal?$('#expectJob .profile_radio li input[value="'+this.typeVal+'"]').attr("checked",!0).parent("li").addClass("current").siblings("li").removeClass("current"):$("#expectJob .profile_radio li input").attr("checked",!1).parent("li").removeClass("current"),$("#expectPosition").val(this.expectPositionVal),
...
var sSite = $("#showSite").html().split("：");
var sJob = $("#showJob").html().split("：");
var sTT = $("#exTime").html().split("：");
$(".expectEdit #expectForm #expectCity").val(sSite[1]);
$(".expectEdit #expectForm #select_expectCity").val(sSite[1]);
$(".expectEdit #expectForm #expectJob").val(sJob[1]);
$(".expectEdit #expectForm #freeTime").val(sTT[1]);
j(this.obj)},
...
if($("#showJob").html()==null ||$("#showJob").html().length==0 ){
...
$("#expectForm").validate({
rules:{
expectCity:{required:!0},
expectJob:{required:!0},
freeTime:{required:!0}},
messages:{
expectCity:{required:"请选择期望地点"},
expectJob:{required:"请选择期望兼职类型"},
freeTime:{required:"请输入空余时间"}
},
errorPlacement:function(a,b){
"hidden"==b.attr("type")?a.appendTo($(b).parent()):"radio"==b.attr("type")?a.insertAfter($(b).parents("ul")):a.insertAfter(b)},
submitHandler:function(a){
var c=$('input[name="expectCity"]',a).val(),
d=$('input[name="expectJob"]',a).val(),
f=$('input[name="freeTime"]',a).val();
$(a).find(":submit").attr("disabled",true);
$.ajax({
url:"comExpect",
type:"POST",
data:{exSite:c,exJob:d,freeTime:f,email:email},dataType:"json",
success:function(response){
$(".expectShow").removeClass("dn");
$(".c_edit").removeClass("dn");
$(".expectEdit").addClass("dn");
$("#showSite").html("期待地点："+c);
$("#showJob").html("期待兼职："+d);
$("#exTime").html("空余时间："+f);
$(a).find(":submit").val("保 存").attr("disabled",!1);
}
...
scoreChange(g.content.infoCompleteStatus.score),k(b.obj)}

else b.obj.find(".expectShow").addClass("dn"),
b.obj.find(".expectEdit").addClass("dn"),
b.obj.find(".c_edit").addClass("dn"),
b.obj.find(".expectAdd").removeClass("dn"),k(b.obj);
...
c.obj.children(".experienceShow").on({mouseenter:function(){
...
var a=$.trim($(this).val());""!=a&&suggestCompany(a,".companyName")}),


c.obj.delegate(".positionName","keyup",function(){
var a=$.trim($(this).val());""!=a&&suggestPosition(a,".positionName")}),
...
else alert(b.msg);$(a).find(":submit").val("保 存").attr("disabled",!1)})}})}),
...
d={obj:$("#projectExperience"),resetForm:function(){
...
Edit:function(a){
var m,b=a.parents("li"),c=b.attr("data-id"),
...
m.find(".word_count").children("span").text(500-m.find(".projectDescription").val().length),m.find(".projectId").val(c),
...
d.obj.find(".plist li:last-child").addClass("noborder"),d.obj.find(".projectAdd").bind("click",function(){d.Add()}),
...
d.obj.children(".projectShow").on({mouseenter:function(){$(this).children(".plist").find(".sm_del,.sm_edit").removeClass("dn")},mouseleave:function(){$(this).children(".plist").find(".sm_del,.sm_edit").addClass("dn")}}),

d.obj.on("keyup",".s_textarea",function(){
var a=$(this);$.trim(a.val()).length>500?a.val($.trim(a.val()).substring(0,500)):a.next(".word_count").children("span").text(500-$.trim(a.val()).length)}),
...
a.stopPropagation();var b=$(this).text();selectYear("schoolYearEnd",b),$(this).parents(".educationalForm").validate().element($(".schoolYearEnd"))}),
...
a.stopPropagation();var b=$(this).text();$(".select_degree").val(b).css("color","#333").removeClass("select_focus"),$(".degree").val(b),$(".box_degree").hide(),$(this).parents(".educationalForm").validate().element($(".degree"))}),

$("#basicInfo .educationalAdd").click(function(){
$("#basicInfo .basicEdit").removeClass("dn");
$("#basicInfo .basicShow").addClass("dn");
$("#basicInfo .educationalAdd").addClass("dn");
}),
...
var sSchool = $("#showSchool").html().split("：");
$(".educationalEdit .educationalForm .schoolName").val(sSchool[1]);
var sRank = $("#showRank").html().split("：");
$(".educationalEdit .educationalForm .degree").val(sRank[1]);
$(".educationalEdit .educationalForm .select_degree").val(sRank[1]);

var sTime = $("#showTime").html().split("：");
var bTime = sTime[1].split("-");
$(".educationalEdit .educationalForm .select_schoolYearStart").val(bTime[0]);
$(".educationalEdit .educationalForm .schoolYearStart").val(bTime[0]);

$(".educationalEdit .educationalForm .schoolYearEnd").val(bTime[1]);
$(".educationalEdit .educationalForm .select_schoolYearEnd").val(bTime[1]);


},

Del:function(a){var b=a.parent("li").attr("data-id");$.ajax({url:ctx+"/educationExperience/delEdu.json",type:"POST",data:{id:b}}).done(function(b){b.success?(a.parent("li").remove(),""==b.content.educationExperiences||null==b.content.educationExperiences?(f.obj.children(".educationalShow").addClass("dn"),f.obj.children(".educationalEdit").addClass("dn"),f.obj.children(".educationalAdd").removeClass("dn"),f.obj.children(".c_add").addClass("dn")):f.obj.children(".educationalShow").children(".educationalForm").hide(),$("#lastChangedTime span").text(b.content.refreshTime),$("#resumeScore .which div").text(b.content.infoCompleteStatus.msg),$("#resumeScore .which span").attr("rel",b.content.infoCompleteStatus.nextStage),scoreChange(b.content.infoCompleteStatus.score),k(f.obj),b.content.isNew&&changeAllIds(b.content.jsonIds)):alert(b.msg)})},AddCancel:function(){this.obj.find(".educationalAdd").removeClass("dn"),this.obj.find(".educationalShow").addClass("dn"),this.obj.find(".educationalEdit").addClass("dn"),k(this.obj)},AddMoreCancel:function(){this.obj.children(".educationalShow").children(".educationalForm").hide(),k(this.obj)}},


f.obj.find(".educationalAdd").bind("click",function(){f.Add()}),
f.obj.find(".c_add").bind("click",function(){
f.AddMore(),$(this).addClass("dn")}),
f.obj.on("click",".c_edit",function(){f.Edit($(this))}),
f.obj.on("click",".sm_del",function(){confirm("确认要删除吗？")&&f.Del($(this))}),

f.obj.children(".educationalShow").on({mouseenter:function(){
...
var htm = $(".elist #showSchool").html();
if(htm==null || htm.length<=0){
$("#educationalBackground .c_edit").addClass("dn");
$("#educationalBackground .educationalEdit").addClass("dn");
...
$("#educationalBackground .educationalShow .elist").removeClass("dn");
}
}),
...
$(this).validate({groups:{schoolYear:"schoolYearStart schoolYearEnd"},
rules:{
...
schoolYearStart:{required:"请选择开始年份",checkSchoolYear:"开始年份需小于结束年份"},schoolYearEnd:{required:"请选择结束年份",checkSchoolYear:"结束年份不能早于开始年份"}},errorPlacement:function(a,b){
...
url:"comEduInfo",
type:"POST",
data:{email:email,school:b,rank:c,beginYear:e,endYear:g},
dataType:"json",
success:function(response){
$("#educationalBackground .c_edit").removeClass("dn");
$("#educationalBackground .educationalShow").removeClass("dn");
$("#educationalBackground .educationalShow").children(".educationalForm").addClass("dn");
$("#educationalBackground .educationalEdit").addClass("dn");
$("#educationalBackground .educationalShow .elist").removeClass("dn");
$("#showSchool").html("学校："+b);
$("#showTime").html("时间："+e+"-"+g);
$("#showRank").html("学历："+c);
$(a).find(":submit").val("保 存").attr("disabled",false);
}
}).done(function(b){
var c,d,e;
if($("#resubmitToken").val(b.resubmitToken),b.success){
...
$("#resumeScore .which span").attr("rel",b.content.infoCompleteStatus.nextStage),
scoreChange(b.content.infoCompleteStatus.score),
k(f.obj),b.content.isNew&&changeAllIds(b.content.jsonIds)}
else
$(a).find(":submit").val("保 存").attr("disabled",false)})}})}),
...
this.text=b,this.obj.children(".descriptionEdit").find(".selfDescription").val(b),
...
var htm = $(".descriptionShow").html();
$(".descriptionEdit .descriptionForm input[name='selfDescription']").val(htm);
j(this.obj)},
...
g.obj.children(".descriptionAdd").bind("click",function(){g.Add()}),
...
url:"comDescribe",
type:"POST",
data:{introduce:b,email:email},
dataType:"json",
success:function(response){
$("#selfDescription").children(".c_edit").removeClass("dn");
$(".descriptionShow").html(b).removeClass("dn");
$(".descriptionEdit").addClass("dn");
...
Edit:function(a){
var f,b=a.parents("li"),c=b.attr("data-id"),d=b.find("div.f16 a").text(),e=b.find("p").html();e&&(e=$.trim(e.replace(/<br>/gi,""))),this.resetForm(),f=this.obj.children(".workEdit").children("form").clone(),f.find(".workLink").val(d),f.find(".workDescription").val(e),f.find(".word_count").children("span").text(100-f.find(".workDescription").val().length),f.find(".showId").val(c),a.parents("li").prepend(f),placeholderFn(),a.parents("li").children(".workList").addClass("dn"),i(),j(this.obj)},

Del:function(a){
var b=a.parents("li").attr("data-id");$.ajax({url:ctx+"/workShow/delws.json",type:"POST",data:{id:b}}).done(function(b){b.success?(a.parents("li").remove(),$("ul.slist li:last-child").addClass("noborder"),(""==b.content.workShows||null==b.content.workShows)&&(h.obj.children(".workShow").addClass("dn"),h.obj.children(".workEdit").addClass("dn"),h.obj.children(".workAdd").removeClass("dn"),h.obj.children(".c_add").addClass("dn")),$("#lastChangedTime span").text(b.content.refreshTime),$("#resumeScore .which div").text(b.content.infoCompleteStatus.msg),$("#resumeScore .which span").attr("rel",b.content.infoCompleteStatus.nextStage),scoreChange(b.content.infoCompleteStatus.score),k(h.obj),b.content.isNew&&changeAllIds(b.content.jsonIds)):alert(b.msg)})},
...
mouseenter:function(){$(this).children(".slist").find(".sm_del,.sm_edit").removeClass("dn")},mouseleave:function(){$(this).children(".slist").find(".sm_del,.sm_edit").addClass("dn")}}),h.obj.children(".workEdit").find(".btn_profile_cancel").bind("click",function(){h.AddCancel()}),
h.obj.children(".workShow").on("click",".btn_profile_cancel",function(){h.AddMoreCancel($(this)),h.obj.find(".c_add").removeClass("dn")}),

h.obj.on("keyup",".s_textarea",function(){
var a=$(this);$.trim(a.val()).length>100?a.val($.trim(a.val()).substring(0,100)):a.next(".word_count").children("span").text(100-$.trim(a.val()).length)}),
